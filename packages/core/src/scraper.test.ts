  ...
  function parseStreamLinksFromHtml(html: string) {
    ...
    for (const table of root.querySelectorAll("table.lnktbj")) {
      ...
      if (href.startsWith("acestream://")) {
        ...
        if (m) {
          streams.push({ type: "youtube", url: `https://www.youtube.com/watch?v=${m[1]}`, bitrate, provider: "YouTube" });
        }
...
describe("resolver", () => {
  test("acestream link produces two stream options", () => {
    const links: StreamLink[] = [
      { type: "acestream", url: "acestream://deadbeef".padEnd(50, "0").slice(0, 50), bitrate: "5000kbps", provider: "AceStream" },
    ];
  ...
  test("acestream links are sorted before webplayer links", () => {
    const links: StreamLink[] = [
      ...
      { type: "acestream", url: "acestream://abc123".padEnd(50, "0").slice(0, 50), bitrate: "3000kbps", provider: "AceStream" },
    ];
    const resolved = resolveStreams(links);
    expect(resolved[0].url.startsWith("acestream://") || resolved[0].url.includes("localhost")).toBe(true);
  });

  test("youtube link passes through as-is", () => {
    const links: StreamLink[] = [
      { type: "youtube", url: "https://www.youtube.com/watch?v=abc123defgh", bitrate: null, provider: "YouTube" },
    ];
    const resolved = resolveStreams(links);
